...
# bash command to start snort for eth0
snort_start_eth0 = """
snort -A full -i eth0 -c /etc/snort/snort.conf -D
"""

#bash command to start snort for wlan0
snort_start_wlan0 = """
snort -A full -i wlan0 -c /etc/snort/snort.conf -D
"""

# bash command to start snort for tun0
snort_start_tun0 = """
snort -A full -i tun0 -c /etc/snort/snort.conf -D
"""

# bash command to clear snort logs
snort_clear_logs = """
echo " - Removing old logs"
rm /var/log/snort/alert*
rm /var/log/snort/snort.*
"""

# bash command to check snort logs
snort_check_logs = """
echo " - Snort processes:"
ps -A | grep snort | sed 's/^ *//; 2,$s/^/ /'
echo " "
echo "- Alerts by Priority:"
cat /var/log/snort/alert | grep Priority | sort | uniq -c | sort -r | sed 's/^ *//; 2,$s/^/ /'
echo " "
echo "- Alerts by type:"
cat /var/log/snort/alert | grep [**] | grep -v Ack | sort | cut -d "]" -f 3 | cut -d "[" -f 1 | sort | uniq -c | sort -r | sed 's/^ *//; 2,$s/^/ /'
"""

# bash command to stop snort
snort_stop_all = """
echo " - Killing Snort processes"
for i in $(pgrep snort); do kill -9 $i; done
echo " - Done"
"""

# check snort eth0
snort_check_eth0 = """
ps -aux | grep "snort" | grep eth0 | grep -v grep | cut -d " " -f 7,27,31
"""

# check snort wlan0
snort_check_wlan0 = """
ps -aux | grep "snort" | grep wlan0 | grep -v grep | cut -d " " -f 7,27,31
"""

# check snort tun0
snort_check_tun0 = """
ps -aux | grep "snort" | grep tun0 | grep -v grep | cut -d " " -f 7,27,31
"""
...
# def to check snort presence
def is_snort_running(command):
...
# def to start snort for eth0
def snort_eth0():
    if not is_snort_running(snort_check_eth0):
        run_bash_commands(snort_start_eth0.strip().splitlines())
    else:
        graphic_banner()
        print(bcolors.RED + "\n ***" + bcolors.ENDC + " Snort already running on eth0! " + bcolors.RED + "***" + bcolors.ENDC)
    
# def to start snort for wlan0
def snort_wlan0():
    if not is_snort_running(snort_check_wlan0):
        run_bash_commands(snort_start_wlan0.strip().splitlines())
    else:
        graphic_banner()
        print(bcolors.RED + "\n ***" + bcolors.ENDC + " Snort already running on wlan0! " + bcolors.RED + "***" + bcolors.ENDC)

# def to start snort for tun0
def snort_tun0():
    if not is_snort_running(snort_check_tun0):
        run_bash_commands(snort_start_tun0.strip().splitlines())
    else:
        graphic_banner()
        print(bcolors.RED + "\n ***" + bcolors.ENDC + " Snort already running on tun0! " + bcolors.RED + "***" + bcolors.ENDC)

# def to check snort logs
def snort_logs():
    run_bash_commands(snort_check_logs.strip().splitlines())
    
# def to kill all snort processes
def snort_kills():
    run_bash_commands(snort_stop_all.strip().splitlines())
    
# def to clear snort logs
def snort_delete_all_logs():
    if not is_snort_running(snort_check_eth0):
        if not is_snort_running(snort_check_wlan0):
            if not is_snort_running(snort_check_tun0):
                run_bash_commands(snort_clear_logs.strip().splitlines())        
            else:
                graphic_banner()
                print(bcolors.RED + "\n ***" + bcolors.ENDC + " Snort running, stop it! " + bcolors.RED + "***" + bcolors.ENDC)
        else:
            graphic_banner()
            print(bcolors.RED + "\n ***" + bcolors.ENDC + " Snort running, stop it! " + bcolors.RED + "***" + bcolors.ENDC)
    else:
...
def print_service_status():
    ...
    if not is_snort_running(snort_check_eth0):
        snort_status_eth0 = f"{bcolors.RED}OFF{bcolors.ENDC}"
    else:
        snort_status_eth0 = f"{bcolors.GREEN}ON{bcolors.ENDC}"
    if not is_snort_running(snort_check_wlan0):
        snort_status_wlan0 = f"{bcolors.RED}OFF{bcolors.ENDC}"
    else:
        snort_status_wlan0 = f"{bcolors.GREEN}ON{bcolors.ENDC}"   
    if not is_snort_running(snort_check_tun0):
        snort_status_tun0 = f"{bcolors.RED}OFF{bcolors.ENDC}"
    else:
        snort_status_tun0 = f"{bcolors.GREEN}ON{bcolors.ENDC}"
    ...
    print(f" Snort on eth0 {snort_status_eth0} - Snort on wlan0 {snort_status_wlan0} - Snort on tun0 {snort_status_tun0}")
...
def snort_menu():
    ...
    # option list
    options = {
        '1': 'Start snort on ' + bcolors.GREEN + 'eth0' + bcolors.ENDC + '.',
        '2': 'Start snort on ' + bcolors.GREEN + 'wlan0' + bcolors.ENDC + '.',
        '3': 'Start snort on ' + bcolors.GREEN + 'tun0' + bcolors.ENDC + '.',
        '4': bcolors.RED + 'Kill' + bcolors.ENDC + ' all snort instances.',
        '5': 'Check all Snort logs.',
        '6': bcolors.RED + 'Delete' + bcolors.ENDC + ' old logs.',
        '98': 'Refresh terminal.',
...
def main_menu():
    ...
    options = {
        ...
        '3': 'Snort ' + bcolors.GREEN + 'quick log view' + bcolors.ENDC + '.',
        '4': 'Firewall menu.',
        '5': 'Apache menu.',
        '6': 'SSH menu.',
        '7': 'Fail2ban menu.',
